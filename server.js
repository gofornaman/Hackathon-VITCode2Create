...
const candidates = ['Gandhi', 'Modi', 'Kumar'];
...
.then((accounts) => {
  ...
  .then((result) => {
    ...
    return deployedContract.methods.totalVotesFor(asciiToHex('Gandhi')).call();
  })
  .then((votesRama) => {
    console.log('votesRama', votesRama);
    return deployedContract.methods.voteForCandidate(asciiToHex('Gandhi')).send();
  })
  .then((voteResult) => {
